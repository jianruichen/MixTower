...
class CommonData:
    def __init__(self, __C) -> None:
        ...
        # load bert tokenizer
        self.tokenizer = BertTokenizer.from_pretrained(__C.LARGE_BERT, do_lower_case=True)
...
class DataSet(Data.Dataset):
    def __init__(self, __C, common_data, split_name_list):
        ...
        for split_name in split_name_list:
            ques_list = json.load(open(__C.QUESTION_PATH[split_name], 'r', encoding='utf-8'))
            if 'questions' in ques_list:
                ques_list = ques_list['questions']
            self.ques_list += ques_list
            if split_name in __C.ANSWER_PATH:
                ans_list = json.load(open(__C.ANSWER_PATH[split_name], 'r', encoding='utf-8'))
                if 'annotations' in ans_list:
                    ans_list = ans_list['annotations']
                self.ans_list += ans_list

        # indexing data, note that all question_id is set to str, and all image_id is set to int
    ...
    def __getitem__(self, idx):
        # get question in token ids, image in features, and answer in binary-label vector
        __C = self.__C
        ...
        return torch.as_tensor(img_feat, dtype=torch.float), torch.as_tensor(ques_ids, dtype=torch.long), torch.as_tensor(ans_vec, dtype=torch.float)
