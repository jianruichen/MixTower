...
def _extract_feat(img_path, net, T, save_path):
    try:
        ...
        np.savez(save_path, x=clip_feats)
    except Exception:
        pass
...
def main(__C, dataset):
    ...
    for split in SPLIT_TO_IMGS:
        if split.startswith(dataset):
            img_dir_list.append(__C.IMAGE_DIR[SPLIT_TO_IMGS[split]])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', dest='dataset', help='dataset name, e.g., ok', type=str, default='ok')
