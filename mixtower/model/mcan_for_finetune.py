# ------------------------------------------------------------------------------------------------------------- #
# Description: the definition of A wrapper of MCAN for finetuning with the strategy described in the paper.
# ------------------------------------------------------------------------------------------------------------- #
...
# ## this is okvqa
# A wrapper of MCAN for finetuning with the strategy described in the paper.
# We inherit the parameters of existing answers and append new parameters for the new answers.
class MCANForFinetuneok(MCAN):
    ...
    def forward(self, input_tuple, output_answer_latent=False):
        ...
        proj_feat = torch.cat([proj_feat, self.proj1(answer_latent)], dim=1)
...
# A wrapper of MCAN for finetuning with the strategy described in the paper.
# We inherit the parameters of existing answers and append new parameters for the new answers.
class MCANForFinetunecoco(MCAN):
    def __init__(self, __C, answer_size, base_answer_size=3129):
        super().__init__(__C, base_answer_size)
    ...
    def forward(self, input_tuple, output_answer_latent=False):
        proj_feat, answer_latent = super().forward(input_tuple, output_answer_latent=True)
        proj_feat = self.proj1(answer_latent)
